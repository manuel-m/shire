...
must_haves:
  truths:
    - 'Integration test file exists for invoice detail error handling'
  ...
  artifacts:
    - path: 'services/bff-service/src/routes/invoices.integration.test.ts'
      provides: 'Test scaffold for invoice detail error handling'
      min_lines: 40
  key_links: []
---

<objective>
Create integration test scaffold for invoice detail route error handling. This is a Wave 0 task that provides the test infrastructure before implementation begins.
...
<task type="auto">
  <name>Task 1: Create invoice detail integration test scaffold</name>
  <files>services/bff-service/src/routes/invoices.integration.test.ts</files>
  <action>
    Create integration test file using vitest and supertest. Mount invoicesRouter directly (import from services/bff-service/src/routes/invoices.ts).

    Write test cases that will verify the following behaviors once implementation is complete:
    1. Happy path: GET /api/invoices/:id returns invoice data with clientName and engagementDescription when all services are healthy
    2. Partial success (client only): GET /api/invoices/:id returns invoice data with only clientName when engagement service fails
    3. Partial success (neither): GET /api/invoices/:id returns invoice data without enrichment when both services fail
    4. Complete failure: GET /api/invoices/:id returns HTTP 502 when billing service fails
  ...
  <verify>
    <automated>test -f services/bff-service/src/routes/invoices.integration.test.ts</automated>
  </verify>
  <done>Integration test file created with 6+ test cases, imports invoicesRouter, mocks fetchJson via vi.spyOn</done>
</task>

</tasks>

<verification>
- File exists: `test -f services/bff-service/src/routes/invoices.integration.test.ts`
- File has test cases: `grep -c "describe\|it" services/bff-service/src/routes/invoices.integration.test.ts`
</verification>

<success_criteria>
Integration test scaffold created and ready for TDD implementation in Plan 05.
</success_criteria>

<output>
After completion, create `.planning/phases/02-bff-error-handling/02-bff-error-handling-04a-SUMMARY.md`
</output>
